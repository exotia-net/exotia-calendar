...
public class GuiCalendar {
    ...
    public void fill(Gui gui, GuiTemplate guiTemplate, Player player) {
        ...
        if (!calendarPlayer.canObtain()) gui.setItem(slotsRewards.get(step), buttons.get("wygasly").getButton(player, sounds.getError()));
        else {
            gui.setItem(slotsRewards.get(step), ItemBuilder.from(buttons.get("zwykly_zamkniety").getItem()).asGuiItem(event -> {
                ...
                gui.updateItem(slotsStreak.get(streakDays), buttons.get("streak_aktywny").getButton(player, sounds.getStep()));
                calendarPlayer.addStep(slotsRewards.size());
            }));
        }
        ...
        gui.setItem(slotsRewards.get(slotsRewards.size() - 1), buttons.get("wygasly").getButton(player, sounds.getError()));
        ...
        else if (streakDays == slotsRewards.size() - 1) gui.setItem(slotsRewards.get(slotsRewards.size() - 1), buttons.get("bonusowy_otwarty").getButton(player, sounds.getStep()));
        ...
        if (streakDays == slotsStreak.size()) {
            ...
            for (int slot : slotsStreak) gui.setItem(slot, buttons.get("streak_aktywny").getButton(player, sounds.getStep()));
        }
        else {
