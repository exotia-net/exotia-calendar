...
public class GuiCalendar {
    public static void openCalendar(Player player, CalendarPlayer calendarPlayer, ConfigurationGui configurationGui, ConfigurationMessage configurationMessage) {
        ...
        fillInventory(gui, guiTemplate, calendarPlayer);
        gui.open(player);
    }

    public static void fillInventory(Gui gui, GuiTemplate guiTemplate, CalendarPlayer calendarPlayer) {
        ...
        for (int i = step + 1; i < slots.size(); i++)
            gui.setItem(slots.get(i), ItemBuilder.from(buttons.get("wygasly").getItem()).asGuiItem());
